...
1.$\Omega \in \F$
1.if $A \in \F$, then $A^c := \Omega \setminus A \in \mathcal F$
...
1.3'. if ${A_n : n \geq 1}$ is a sequence of events with $A_n \in \mathcal F$ for every $n \geq 1$, then $A := \bigcap_{n \geq 1} A_n \in \mathcal F$
